...
function doPost(e) {
    ...
    try {
        ...
        if (!orderData || !orderData.orderId || !orderData.items || orderData.items.length === 0) {
            throw new Error("Invalid or empty order data received. 'orderId' and 'items' are required.");
        }

        // 3. Record the order to a Google Sheet
        const recordStatus = recordOrderToSheet(orderData);
        if (!recordStatus.success) {
            throw new Error(recordStatus.message || "Failed to record order to sheet.");
        }
        
        responseMessage = "Order received and recorded successfully for Order ID: " + orderData.orderId;
        ...
        Logger.log("Error in doPost: " + error.toString() + (e.postData ? ". Received payload: " + e.postData.contents : ". No payload received."));
...
function recordOrderToSheet(orderData) {
    try {
        ...
        if (sheet.getLastRow() === 0) {
            const headers = ["Timestamp", "Order ID", "Customer Name", "Customer Phone", "Total Amount", "Items JSON", "Raw Data JSON"];
        ...
        const rowData = [timestamp, orderId, customerName, customerPhone, totalAmount.toFixed(2), itemsJson, rawDataJson];
        
        // 4. Append the new order data to the sheet
        sheet.appendRow(rowData);
        Logger.log("Order " + orderId + " recorded to Google Sheet in spreadsheet " + SPREADSHEET_ID);
        return { success: true };

    } catch (error) {
        Logger.log("Error in recordOrderToSheet: " + error.toString() + ". Data: " + JSON.stringify(orderData));
        // Optionally, re-throw or handle error (e.g., send an email notification)
        return { success: false, message: "Failed to record to sheet: " + error.toString() };
    }
}
...
function testRecordOrder() {
    ...
    const sampleOrder = {
        ...
        items: [
            { nome: "Pizza Test Margherita", quantity: 2, preco: 26.90, emoji: "🍕" },
            { nome: "Coca-Cola Test", quantity: 4, preco: 5.00, emoji: "🥤" }
        ],
    ...
    if (SPREADSHEET_ID === "YOUR_SPREADSHEET_ID_HERE") {
        Logger.log("ERROR: SPREADSHEET_ID is not set. Please update the global constant SPREADSHEET_ID in the script.");
        Browser.msgBox("Configuration Error", "SPREADSHEET_ID is not set. Please update it in the script before testing.", Browser.Buttons.OK);
        return;
    }
    ...
    if (status.success) {
        Logger.log("testRecordOrder completed successfully. Order should be in the sheet.");
        Browser.msgBox("Test Complete", "testRecordOrder ran. Check the sheet: " + SPREADSHEET_ID, Browser.Buttons.OK);
    } else {
        Logger.log("testRecordOrder failed: " + status.message);
        Browser.msgBox("Test Failed", "testRecordOrder failed: " + status.message, Browser.Buttons.OK);
    }
}
